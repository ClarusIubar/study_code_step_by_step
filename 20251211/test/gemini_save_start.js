(function main() {
    ...
    // =================================================================
    // 0. [사용자 설정 영역] 이 3줄만 수정하여 원하는 전략을 선택하세요!
    // =================================================================
    
    // 1. 출력 전략: 'CLIPBOARD' (클립보드 복사) 또는 'SPLIT_FILES' (파일 다운로드)
    const OUTPUT_STRATEGY = 'CLIPBOARD';
    
    // 2. 출력 포맷: 'NOTION' (/page 문법) 또는 'OBSIDIAN' (콜아웃 문법)
    const OUTPUT_FORMATTER = 'NOTION';
    
    // 3. 클립보드 방식: 'PAGES' (노션용 하위 페이지 생성) 또는 'SINGLE' (옵시디언용 단일 페이지 복사)
    // NOTE: OUTPUT_FORMATTER가 'NOTION'일 때는 자동으로 'PAGES'가 강제됩니다.
    const CLIPBOARD_STRATEGY = 'PAGES'; 
    
    
    // =================================================================
    // 0. [내부 설정] 내부 사용 상수 및 스타일 (자주 수정할 필요 없음)
    // =================================================================
    
    // 내부 상수들을 정의하고 OUTPUT_STRATEGY/OUTPUT_FORMATTER를 여기에 통합합니다.
    const INTERNAL_CONSTANTS = {
        // 사용자 설정 상수를 내부 상수에 통합 (코드 내에서 일관성 있게 사용)
        ...
        STATUS_START: "⬇️ 데이터 수집 및 페이지 생성 준비 중...", 
        STATUS_FORMATTING: "🧹 마크다운 포맷팅 및 클립보드 복사 준비 중...", 
        STATUS_DOWNLOAD: "💾 파일 분할 완료. 다운로드 시작!", 
        STATUS_SUCCESS: "✅ 완료!", 
        
        CLOSE_TEXT: "닫기 X",
        ALERT_SUCCESS_COPY: (total) => `✅ 복사 완료! (${total}개 대화 쌍)\n\n노션에 붙여넣기 시, 하위 페이지로 생성됩니다.`,
        ALERT_SUCCESS_SPLIT: (total) => `✅ ${total}개의 대화 쌍 파일이 생성되어 다운로드되었습니다.`,
        ALERT_FAIL: "복사 실패",
    ...
    const CONSTANTS = INTERNAL_CONSTANTS; // CONSTANTS는 이제 내부 상수를 가리킵니다.

    /** 프로그램의 실행 중 상태를 관리하는 그룹 객체 */
    let STATE_MANAGER = {
    ...
    // --- 1.1. 전략 및 포맷터의 충돌 정책 조정 ---
    // (CONSTANTS의 값을 최종적으로 보정합니다.)
    if (CONSTANTS.OUTPUT_STRATEGY === 'CLIPBOARD' && CONSTANTS.OUTPUT_FORMATTER === 'NOTION') {
        // 노션 클립보드 사용 시, 무조건 PAGES 전략을 사용하도록 강제
        CONSTANTS.CLIPBOARD_STRATEGY = 'PAGES';
    } else if (CONSTANTS.OUTPUT_STRATEGY === 'CLIPBOARD' && CONSTANTS.OUTPUT_FORMATTER === 'OBSIDIAN') {
        // 옵시디언 클립보드 사용 시, SINGLE 전략을 사용하도록 강제
        CONSTANTS.CLIPBOARD_STRATEGY = 'SINGLE';
    }
    ...
    /** 노션 /페이지 문법을 사용하여 턴을 포맷합니다. (CLIPBOARD_PAGES 핵심) */
    function formatToNotionPage(turnType, text, turnIndex) {
        const title = `Turn ${turnIndex}: ${turnType}`;
        return `/page ${title}\n\n${text}\n\n\n`; 
    }
    
    const FORMATTER_MAP = {
        'OBSIDIAN': formatToObsidianCallout,
        // Notion 포맷터는 필요에 따라 generateFormattedMarkdown에서 직접 호출됩니다.
    };
    ...
    function getFormattedMetadata(title, url) {
        ...
        const formatter = CONSTANTS.OUTPUT_FORMATTER === 'OBSIDIAN' ? formatToObsidianCallout : (t, text) => text;
        const metadataText = `- **Date**: ${today}\n- **Source**: [Link](${url})`;
        
        // 옵시디언 포맷일 경우에만 콜아웃을 사용하여 메타데이터 생성
        if (CONSTANTS.OUTPUT_FORMATTER === 'OBSIDIAN') {
            return `# ${title}\n\n` + formatter('Metadata', metadataText);
        } else {
             // 노션 포맷은 메타데이터 없이 시작 페이지에서 처리
            return "";
        }
    }
    
    /**
     * 순수 함수: 전체 대화 턴 배열을 포맷팅하여 최종 마크다운을 생성합니다.
     */
    async function generateFormattedMarkdown(turns, title) {
        let md = "";
        let totalTurns = 0;
        
        const formatterFn = FORMATTER_MAP[CONSTANTS.OUTPUT_FORMATTER] || FORMATTER_MAP['OBSIDIAN'];
        
        // --- 3.1. 메타데이터/시작 페이지 처리 ---
        if (CONSTANTS.CLIPBOARD_STRATEGY === 'PAGES') {
            // 노션 페이지 분할 복사: 최상단 페이지로 시작
            md += `/page ${title} (Imported Chat)\n\n\n`;
        } else {
            // 단일 복사 또는 파일 분할: 메타데이터 추가
        ...
        for (let i = 0; i < totalTurns; i++) {
            ...
            if (CONSTANTS.CLIPBOARD_STRATEGY === 'PAGES') {
                // 노션 페이지 분할: 사용자-제미나이 쌍을 한 페이지에 담기
                const combinedText = `**User:**\n\n${userText}\n\n---\n\n**Gemini:**\n\n${modelText}`;
                md += formatToNotionPage("Conversation", combinedText, i + 1);
            } else {
                // 단일/파일 분할 (옵시디언)
    ...
    function setupUIPanel() {
        ...
        // 버튼 텍스트는 선택된 전략에 따라 결정
        let btnText = "실행";
        if (CONSTANTS.OUTPUT_STRATEGY === 'CLIPBOARD') {
             btnText = CONSTANTS.OUTPUT_FORMATTER === 'NOTION' ? '📥 노션 하위 페이지로 복사' : '📥 옵시디언 마크다운 복사';
        } else {
             btnText = `💾 ${CONSTANTS.OUTPUT_FORMATTER} 파일 분할 다운로드`;
        }
    ...
    /** 메인 실행 로직: 전략에 따라 클립보드 복사 또는 파일 분할을 수행 */
    async function handleExecution() {
        ...
        } else if (CONSTANTS.OUTPUT_STRATEGY === 'SPLIT_FILES') {
            // 파일 분할 다운로드
            try {
