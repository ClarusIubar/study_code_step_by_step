...
class LottoBall(BaseBall):
    def __init__(self, canvas, number, x, y):
        radius = cfg.LottoConfig.BALL_RADIUS
        dx, dy = random.uniform(-3, 3), random.uniform(3, 6)
        super().__init__(canvas, radius, dx, dy, x, y)
...
#         self.radius = cfg.LottoConfig.BALL_RADIUS
